...
import { changePasswordUserForAdmin } from "@/api/src/pengguna";
import { toast } from "react-toastify";
import { useAppState } from "@/context/AppStateContext";
import { useUsersAccState } from "@/context/UsersAccContext";
import { useAdminState } from "@/context/AdminContext";

export default function ModalChangePasswordUserForAdmin({ userId, userEmail }) {
  ...
  const { password, verifyPassword, loading } = state;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await changePasswordUserForAdmin(currentAdmin.id, {
        password,
        verifyPassword,
        userId,
      });
      ...
      toast.success("Password berhasil diubah", { theme: "colored" });
    } catch (error) {
      console.log(error);
    ...
    <main className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 shadow-2xl lg:w-[30%] md:w-[60%] sm:w-[80%] w-[90%] max-h-screen overflow-auto">
      <div className="w-full mx-auto flex items-center justify-center">
        <div className="w-full bg-gray-200 dark:bg-gray-900 rounded-2xl flex flex-col items-center justify-center p-5 py-20 sm:py-18">
          <div className="flex flex-col items-center mb-2 lg:text-2xl md:text-2xl font-semibold text-teal-800 dark:text-white">
            Reset Password
          </div>
          <p className="lg:text-sm md:text-sm sm:text-sm text-xs text-teal-800 dark:text-white mb-8">
            Reset password untuk email {userEmail}
          </p>
...
ModalChangePasswordUserForAdmin.propTypes = {
